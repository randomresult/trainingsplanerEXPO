...
- `components/sheets/TrainingPickerSheet.tsx` — extend with `presentSeries()` for adding whole series
...
**Create:**
- `lib/queries/useMethodicalSeries.ts` — list + detail queries
...
## Task 3: `useMethodicalSeries` query

**Files:**
- Create: `lib/queries/useMethodicalSeries.ts`
...
```bash
git add lib/queries/useMethodicalSeries.ts
git commit -m "feat(queries): add useMethodicalSeries + useMethodicalSeriesDetail"
```
        ...
        seriesLoading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }: { item: MethodicalSeries }) => (
              // C-1 card design: category chip top-left · progress pill top-right (hidden for now)
              // Name + goal · divider · exercise count + circular + button
              // Progress pill (X/Y) requires cross-training PlayerProgress query — deferred, hidden initially
              <Pressable
              ...
              >
                ...
                <View className="flex-row items-center justify-between">
                  ...
                  <Pressable
                    onPress={(e) => {
                      e.stopPropagation?.();
                      // Note: TrainingPickerSheet must be extended to handle series (Task 6b)
...
## Task 6b: Extend `TrainingPickerSheet` for series

**Files:**
- Modify: `components/sheets/TrainingPickerSheet.tsx`

The existing sheet shows a list of trainings and calls `useAddExerciseToTraining` when one is picked. It needs a parallel flow for adding a whole series (calls `useAddMethodicalSeriesToTraining`).

- [ ] **Step 1: Read the current sheet**

Read `components/sheets/TrainingPickerSheet.tsx` to understand the current API and internal structure.

- [ ] **Step 2: Add `presentSeries` to the ref type**

Add `presentSeries(seriesDocumentId: string, seriesName: string, exerciseDocumentIds: string[]): void` to the `TrainingPickerSheetRef` interface.

- [ ] **Step 3: Implement `presentSeries`**

Inside the sheet, add state to track whether we're adding an exercise or a series:

```typescript
type PickMode =
  | { kind: 'exercise'; exerciseId: string; name: string }
  | { kind: 'series'; seriesId: string; name: string; exerciseIds: string[] };
```

`present(exerciseId, name)` sets `{ kind: 'exercise', ... }`, `presentSeries(...)` sets `{ kind: 'series', ... }`. When a training is picked, branch on `pickMode.kind`:
- `exercise` → existing `useAddExerciseToTraining` mutation
- `series` → `useAddMethodicalSeriesToTraining` mutation

- [ ] **Step 4: Import and use `useAddMethodicalSeriesToTraining`**
...
git commit -m "feat(sheets): extend TrainingPickerSheet to support adding a whole series"
```
  ...
  return (
    <Screen scroll>
      ...
      <View className="px-5 pt-4 pb-8">
        ...
        {/* Per-exercise completion indicators derived from PlayerProgress.
            For now: all shown as ○ (not done). Cross-training progress query is deferred.
            When implemented, replace `false` below with a real completion check. */}
        {(series.exercises ?? []).map((ex, idx) => {
          const done = false; // TODO: derive from PlayerProgress cross-training query
          return (
            <Pressable
              key={ex.documentId}
              onPress={() => router.push({ pathname: '/exercise-detail/[id]', params: { id: ex.documentId } })}
            ...
            >
              ...
              <Text variant="subhead" className="flex-1" numberOfLines={2}>{ex.Name}</Text>
              <View
